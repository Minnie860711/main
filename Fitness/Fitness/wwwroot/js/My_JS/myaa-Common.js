...
//變更表單Url。  (直接收Url)
function myaa_FormChangeAction_ByUrl(form, url) {
    //var form = $("#" + formId);
    $(form).attr("action", url); //變更Form submit時的連結
}

//變更表單Url，並提交。  (直接收Url)
function myaa_FormChangeAction_ByUrl_GoSubmit(formId, url) {
    var form = $("#" + formId);
    form.attr("action", url); //變更Form submit時的連結
    form.submit();
}
...
function myaa_FormChangeAction_ByAction_GoSubmit(formId, controller, action) {
    var form = $("#" + formId);
...
function myaa_ComAlert(_title, _text, _html, _url) {
    ...
    // Log的超連結元素
    var logUrlElm = myaa_IsNullOrEmpty(_url) ? "" : "<a href='" + _url + "' style='font-size: 20px' target='_blank'>完整訊息(開新視窗)</a>";
